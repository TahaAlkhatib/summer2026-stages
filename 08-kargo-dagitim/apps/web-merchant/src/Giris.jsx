...
export default function Giris({ onGiris }) {
  ...
  async function girisYap(e) {
    ...
    try {
      ...
      // Bu portal sadece tacirler için
      if (cevap.user.role !== 'tacir') {
        setHata('Bu portal tacir hesapları içindir. Personel şube uygulamasını kullanmalıdır.')
        setBekliyor(false)
        return
      }
    ...
    <div className="giris-zemin">
      <form className="giris-kutu" onSubmit={girisYap}>
        <div className="giris-ust">
          <h1>Hızlı Kargo — Tacir Portalı</h1>
          <p>Gönderi takibi ve kapıda ödeme mutabakatı</p>
        </div>
        <div className="giris-govde">
          ...
          <p className="kucuk soluk" style={{ textAlign: 'center', marginTop: 16 }}>
            Demo: <b>tacir1 / 123456</b> (Moda Butik Giyim)
          </p>
        </div>
      </form>
    </div>
